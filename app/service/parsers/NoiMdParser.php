...
class NoiMdParser extends BaseParser
{
    /**
     * Конструктор класса.
     *
     ...
     */
    protected $baseUrl = "https://noi.md";
    private $imageUploader;

    public function __construct($url)
    {
        parent::__construct($url,"noimd.log");
    ...
    protected function parse(\Symfony\Component\DomCrawler\Crawler $crawler)
    {
        try {
            ...
            $newsUrlNode = $crawler->filterXPath("(//a[@class='news-feed-item-hdr'])[1]");
            ...
            $logoUrlNode = $crawler->filterXPath("(//div[@class='row'])[2]//a");
            ...
            $newsTitleNode = $crawler->filterXPath("//div[@class='row news-head']//h1");
            ...
            $bodyNode = $crawler->filterXPath("(//div[@class='row news-text'])[1]//p");
            if ($bodyNode->count() === 0) {
